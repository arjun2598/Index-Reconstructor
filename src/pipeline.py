...
def build(refresh=False):
    print("raw:")
    stocks, changes, prices, raw_shares, raw_benchmark, splits = load_raw(refresh)

    # Clean layer is cheap to rebuild, so we always redo it rather than caching a stale version
    print("clean:")
    closes, shares, benchmark, membership = build_clean(stocks, changes, prices, raw_shares, raw_benchmark, splits)
